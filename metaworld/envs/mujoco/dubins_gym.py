...
class DubinGym(gym.Env):
	...
	def __init__(self, start_point, target_point, sample_goal=False):
		...
		low = np.array([-1.,-1.,-4., -1.,-1.,-4.])
		high = np.array([1.,1.,4., -1.,-1.,-4.])
	...
	def set_goal(self, goal):
		target = [goal[0]/MAX_X, goal[1]/MAX_Y, 1.57]
		self.target = target
	...
	def reset(self): 
		...
		if self.sample_goal:
			target = np.random.uniform(np.array([0., 2.]), np.array([5., 5.]), size=(2,))
			target = [target[0]/MAX_X, target[1]/MAX_Y, 1.57]
			self.target = target
	...
	def _get_obs(self):
		return np.concatenate([self.pose, self.target]).copy()
